...
export default function BrandRowActions({ brand, onUpdate, onNotify }) {
    ...
    const handleEdit = () => navigate(`/marcas/editar/${brand.id}`);
    const handleView = () => navigate(`/marcas/visualizar/${brand.id}`);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            onNotify({ severity: "success", message: "Marca eliminada exitosamente" });
            onUpdate((prev) => prev.filter((b) => b.id !== brand.id));
        } catch {
            onNotify({ severity: "error", message: "Error al eliminar la marca" });
        }
    };
